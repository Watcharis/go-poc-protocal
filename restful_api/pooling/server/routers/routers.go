...
func InitRouter(ctx context.Context, handler handlers.PoolingServerHandlers) http.Handler {
	e := echo.New()
	...
	api := e.Group("/api")
	v1 := api.Group("/v1")
	v1.POST("/tricker-pooling-server", handler.ServerRecieveTricker)
	v1.POST("/get-status", handler.GetStatus)
